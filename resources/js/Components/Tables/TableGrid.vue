...
const getStatusBadge = (cell) => {
    const statusMap = {
        ...
        'desbloqueado': { class: 'bg-success-subtle text-success', text: 'Desbloqueado' },
        'inativo': { class: 'bg-danger-subtle text-danger', text: 'Inativo' },
        'bloqueado': { class: 'bg-danger-subtle text-danger', text: 'Bloqueado' },
        ...
        gridColumns.push({
            ...
            formatter: (cell, row) => {
                ...
                const buttons = [
                    ...
                    can(ac.bank) ? `<button class="btn btn-sm btn-soft-secondary" type="button" data-action="bank" data-id="${rowId}" data-row='${rowDataStr}' data-loading="${isLoadingAction('bank') ? 'true' : 'false'}" ${(disabledAll || isRowLoading) ? 'disabled' : ''} title="${al.bank ?? 'Gateways de Pagamento'}">${isLoadingAction('bank') ? `<span class="spinner-border spinner-border-sm me-1" role="status" aria-hidden="true"></span>` : ''}<i class="${ai.bank ?? 'ri-bank-card-line'} align-bottom ${isLoadingAction('bank') ? 'd-none' : ''}"></i>${bt.bank ? `<span class="d-none d-sm-inline ms-1">${bt.bank}</span>` : ''}</button>` : ``,
