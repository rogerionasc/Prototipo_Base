        ...
        return {
            ...
            menuItems: [
                ...
                },
                {
                    ...
                    children: [
                        { href: "/parametrizacao/sistema", label: "Sistema" },
                        { href: "/parametrizacao/tiss", label: "TISS" },
                    ]
                },
                {
            ...
            <ul class="navbar-nav h-100" id="navbar-nav">
                <template v-for="(item, index) in menuItems" :key="index">
                    ...
                    <li v-else-if="item.type === 'link'" class="nav-item">
                        <a v-if="item.children" class="nav-link menu-link" :href="item.target" data-bs-toggle="collapse"
                            role="button" aria-expanded="false" :aria-controls="item.target.replace('#', '')">
                            <i :class="item.icon"></i>
                            <span>{{ item.title }}</span>
                        </a>

                        <Link v-else class="nav-link menu-link"
                            ...
                            <ul class="nav nav-sm flex-column">
                                <template v-for="(child, cidx) in item.children" :key="cidx">
                                    ...
                                    <li v-else-if="child.children" class="nav-item">
                                        <a class="nav-link" :href="child.target" data-bs-toggle="collapse" role="button"
                                            aria-expanded="false" :aria-controls="child.target.replace('#', '')">
                                            {{ child.title }}
                                        </a>

                                        <div class="collapse menu-dropdown" :id="child.target.replace('#', '')">
                                            <ul class="nav nav-sm flex-column">
                                                <li v-for="(sub, subIdx) in child.children" :key="subIdx"
                                                    class="nav-item">
