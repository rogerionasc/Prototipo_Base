        ...
        <div class="row">
            <div class="col-lg-12">
                <SimpleTable
                    ...
                    emptyMessage="Não há pacientes na lista atual."
                ...
                >
                    ...
                    <template #top>
                        <ul class="nav nav-tabs nav-tabs-custom nav-success mb-3" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link fw-semibold" :class="{ active: activeTab === 'todos' }" @click.prevent="activeTab = 'todos'" href="#">
                                    Todos <span class="badge bg-secondary align-middle ms-1">{{ atendimentos.length }}</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link fw-semibold" :class="{ active: activeTab === 'aguardando' }" @click.prevent="activeTab = 'aguardando'" href="#">
                                    Aguardando <span class="badge bg-warning align-middle ms-1">{{ totalAguardando }}</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link fw-semibold" :class="{ active: activeTab === 'em_atendimento' }" @click.prevent="activeTab = 'em_atendimento'" href="#">
                                    Em Atendimento <span class="badge bg-primary align-middle ms-1">{{ totalEmAtendimento }}</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link fw-semibold" :class="{ active: activeTab === 'atendidos' }" @click.prevent="activeTab = 'atendidos'" href="#">
                                    Finalizados <span class="badge bg-success align-middle ms-1">{{ totalAtendidos }}</span>
                                </a>
                            </li>
                        </ul>
                    </template>
                    ...
                    <template #cell(paciente)="{ item }">
                        <div class="d-flex align-items-center">
                            <div class="flex-shrink-0">
                                <div class="avatar-xs">
                                    <div class="avatar-title rounded-circle shadow" :class="getRandomColorClass(item.paciente?.nome)">
                                        {{ getInitials(item.paciente?.nome) }}
                                    </div>
                                </div>
                            </div>
                            <div class="flex-grow-1 ms-2 name">
                                <h5 class="fs-14 mb-0">{{ item.paciente?.nome || 'N/A' }}</h5>
                                <p class="text-muted mb-0 fs-12" v-if="item.paciente?.cpf">CPF: {{ item.paciente.cpf }}</p>
                            </div>
                        </div>
                    </template>
                    ...
                    <template #cell(detalhes)="{ item }">
                        <div class="d-flex flex-column">
                            ...
                            <span class="text-muted fs-12">Dr(a). {{ item.medico?.nome || 'N/A' }}</span>
                        </div>
                    </template>
                    ...
                    <template #cell(chegada)="{ item }">
                        <div class="d-flex flex-column">
                            ...
                            <span class="text-muted fs-12 fw-medium"><i class="ri-time-line align-bottom text-primary"></i> Previsto: {{ item.hora_prevista }}</span>
                        </div>
                    </template>
                    ...
                    <template #actions="{ item }">
                        <div class="d-flex gap-2">
                            <template v-if="item.status === 'AGUARDANDO' || item.status === 'CHAMADO'">
                                <Link 
                                    v-if="!hasAtendimentoEmAndamento(item.medico_id)"
                                    :href="route('atendimentos.chamar', item.id)" 
                                    method="post" 
                                    as="button" 
                                ...
                                <Link 
                                    v-if="!hasAtendimentoEmAndamento(item.medico_id)"
                                    :href="route('atendimentos.iniciar', item.id)" 
                                    method="post" 
                                    as="button" 
                            ...
                            <template v-else-if="item.status === 'EM ATENDIMENTO'">
                                <Link 
                                    :href="route('atendimentos.pep', item.id)" 
