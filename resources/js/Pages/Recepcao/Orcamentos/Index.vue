<template>
    <Layout>
        ...
        <PageHeader title="Orçamento" pageTitle="Recepção" />
            ...
            <div class="col-lg-8">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select ref="selProcedimento" v-model="selectedProcId" class="form-select" data-choices
                                :disabled="locked || procedimentosSelectLoading" @change="onSelectProcedure" required
                                :class="{ 'is-invalid': !!form.errors.itens }">
                                <option value="">Digite para buscar por nome ou código...</option>
                                <option v-for="p in procedimentosSelectRows" :key="`${p.source || ''}:${p.id}`"
                                    :value="`${p.source || ''}:${p.id}`">{{ p.descricao ? `${p.nome} - ${p.descricao}` :
                                    p.nome }}</option>
                            </select>
                            <div v-if="form.errors.itens" class="invalid-feedback d-block mt-2">{{ form.errors.itens }}
                            </div>
                        </div>

                        <!-- Invoice Table -->
                        <SimpleTable variant="borderless" tableClass="table-bordered" :items="itensLocal"
                            :columns="procedimentosColumns" emptyTitle="Nenhum procedimento"
                            emptyMessage="Nenhum procedimento adicionado ao orçamento." emptyIcon="ri-file-list-3-line">
                                ...
                                <template v-for="(it, idx) in items" :key="`group-${idx}`">
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <span class="text-muted small">Convênio: {{ selectedConvenioRow?.descricao
                                                || '—' }}</span>
                                        </td>
                                        <td class="text-end">{{ formatCurrency(it.valor_unitario) }}</td>
                                        <td class="text-end fw-medium">{{ formatCurrency(it.valor_total) }}</td>
                                        <td class="text-center">
                                            <button class="btn btn-sm btn-soft-danger" type="button" :disabled="locked"
                                                @click="removeItem(idx)" title="Remover">
                                    ...
                                    <tr v-for="n in sessionCount(it)" :key="`sess-${idx}-${n}`">
                                        <td colspan="4" class="py-2 ps-4">
                                            <div class="d-flex align-items-center text-muted small">
                                                ...
                                                <span class="badge bg-light text-dark border me-2">Sessão {{ n }}/{{
                ...
                <div class="card mt-3">
                    ...
                    <div class="card-body">
                        ...
                        <button class="btn btn-success w-100" type="button" :disabled="saveProcessing || locked"
                            @click="save">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="vstack gap-2" v-if="ultimosLocal && ultimosLocal.length > 0">
                            <div v-for="o in ultimosLocal" :key="o.id"
...
import OrcamentoPrint from "@/Pages/Recepcao/Orcamentos/OrcamentoPrint.vue";
